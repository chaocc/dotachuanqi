 ...
 *
 *      $ Id: Exjson.php UTF-8 2013-11-26 下午3:01:29Z Shalom $
 */
 ...
 * 
 * @author Shalom
 * @example $this->load->library('exjson'); $this->exjson->set_ret('data', '保存成功，请稍候再试');  $this->exjson->set_ret('login', TRUE);  $this->exjson->output();
 */
...
class Exjson {
	 ...
	 * 返回结果值
	 */
	var $ret = array('result' => FALSE, 'errormsg' => '', 'data' => '');

	/**
	 * 作为  exjson 类对象的副本
	 */
	private static $obj_json = NULL;
		...
		switch (gettype($var)) {
			...
			case 'array':
				...
				if (is_array($var) && count($var) && (array_keys($var) !== range(0, sizeof($var) - 1))) {
					return '{' .
							join(',', array_map(array(self::$obj_json, 'nameValue'),
									array_keys($var),
									array_values($var)))
									. '}';
				}
				...
				return '[' . join(',', array_map(array(self::$obj_json, 'encode'), $var)) . ']';
			...
			case 'object':
				...
				if (interface_exists('JsonSerializable', false) && $var instanceof JsonSerializable)
				{
					...
					return $this->encode($var->jsonSerialize());
				}
	 ...
	 * @access   private
	 */
	private function nameValue($name, $value)
	{
	 ...
	 * @access   private
	 */
	private function reduceString($str)
	{
					...
					for ($c = 0; $c <= $strlen_chrs; ++$c) {
						...
						if (($c == $strlen_chrs) || (($chrs{$c} == ',') && ($top['what'] == self::JSON_SLICE))) {
							...
							if (reset($stk) == self::JSON_IN_ARR) {
								...
								$arr[] = $this->decode($slice,$useArray);
								...
								if (preg_match('/^\s*(["\'].*[^\\\]["\'])\s*:\s*(\S.*),?$/Uis', $slice, $parts)) {
									// "name":value pair
									$key = $this->decode($parts[1],$useArray);
									$val = $this->decode($parts[2],$useArray);
	 ...
	 * @return string
	 */
	private function utf8ToUTF16BE(&$str, $bom = false)
	{
		$out = $bom ? "\xFE\xFF" : '';
		if(function_exists('mb_convert_encoding'))
			return $out.mb_convert_encoding($str,'UTF-16BE','UTF-8');
	
		$uni = $this->utf8ToUnicode($str);
		foreach($uni as $cp)
			$out .= pack('n',$cp);
		return $out;
	}
	 ...
	 * @return string
	 */
	private function utf16beToUTF8(&$str)
	{
		$uni = unpack('n*',$str);
		return $this->unicodeToUTF8($uni);
	}
	 ...
	 * @param unknown $value
	 */
	public function set_ret($field, $value)
	{
		$field = strtolower(trim(strip_tags($field)));
		if ( ! $field || ! is_string($field) ) {
			return FALSE;
		}
			
		$this->ret[$field] = $value;
	 ...
	 * @param string $halt
	 */
	public function output($ret = NULL, $halt = TRUE)
	{
		if ($ret === NULL) {
			$ret = $this->encode($this->ret);
		}
		echo $ret;
		if ($halt === TRUE) {
			exit();
	 ...
	 * 初始化 exjson 对象副本
	 */
	private function _init_exjson()
	{
		if ( self::$obj_json === NULL ) {
			self::$obj_json = new Exjson();
		}
		
		return (self::$obj_json !== NULL);
	}
	
}

/* End of file Exjson.php */
/* Location: ./application/libraries/Exjson.php */