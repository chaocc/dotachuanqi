 ...
 *
 *      $ Id: Exiconv.php UTF-8 2013-11-26 下午2:23:36Z Shalom $
 */
defined('BASEPATH') OR exit('No direct script access allowed');

! defined('CODETABLE_DIR') && define('CODETABLE_DIR', APPPATH.'libraries/iconvtable/');
! defined('ICONV_ENABLE') && define('ICONV_ENABLE', function_exists('iconv'));
! defined('MB_ENABLE') && define('MB_ENABLE', function_exists('mb_convert_encoding'));

/**
 * @example $this->load->library('exiconv'); $this->exiconv->diconv('这是一个简单的测试哦', 'utf-8', 'big5');
 * @author Shalom
 *
 */
...
class Exiconv {
	...
	private $table = '';
	private $iconv_enabled = FALSE;
	private $convertbig5 = FALSE;
	 ...
	 * 字符串编码转换
	 * @param unknown $str
	 * @param unknown $in_charset
	 * @param string $out_charset
	 * @param string $ForceTable
	 */
	public function diconv($str, $in_charset, $out_charset = 'UTF-8', $ForceTable = FALSE)
	{
		...
		if($out == '') {
			$this->Chinese($in_charset, $out_charset, TRUE);
			$out = $this->Convert($str);
		}
	 ...
	 * @param string $ForceTable
	 */
	private function Chinese($SourceLang, $TargetLang, $ForceTable = FALSE) {
	...
	private function GB2312toBIG5($c) {
	...
	private function Convert($SourceText) {
	...
	private function Utf8_Unicode($char) {
		switch(strlen($char)) {
			case 1:
				return ord($char);
			case 2:
...
/* End of file Exiconv.php */
/* Location: ./application/libraries/Exiconv.php */